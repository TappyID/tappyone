...
export const useAtendimentoStates = () => {
  ...
  // 🆕 ACEITAR ATENDIMENTO
  const aceitarAtendimento = useCallback(
    async (chatId: string, filaId?: string) => {
  ...
  const transferirAtendimento = useCallback(
    async (
      chatId: string,
  ...
  const listarChatsPorStatus = useCallback(
    async (
  ...
  const buscarEstatisticas = useCallback(
    async (filaId?: string, responsavelId?: string) => {
      setLoading(true);
      try {
        if (!token) throw new Error("Token não disponível");
        const params = new URLSearchParams();
  ...
  const buscarStatusChat = useCallback(
    async (chatId: string) => {
      try {
        if (!token) throw new Error("Token não disponível");
        const encodedChatId = encodeURIComponent(chatId);
