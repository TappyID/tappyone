...
export async function POST(request: NextRequest, { params }: { params: { sessionName: string } }) {
  try {
    console.log(`🛑 [WAHA PROXY] POST /sessions/${params.sessionName}/stop - Parando sessão`)

    const response = await fetch(`${WAHA_API_URL}/api/sessions/${params.sessionName}/stop`, {
      method: 'POST',
      headers: {
    ...
    console.log('✅ [WAHA PROXY] Sessão parada:', data)
