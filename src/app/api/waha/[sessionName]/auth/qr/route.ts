...
export async function GET(request: NextRequest, { params }: { params: { sessionName: string } }) {
  try {
    console.log(`📱 [WAHA PROXY] GET /${params.sessionName}/auth/qr - Obtendo QR Code`)

    // Construir URL com query params
    const url = new URL(request.url)
    const queryParams = new URLSearchParams()
    
    // Adicionar query params da requisição original
    url.searchParams.forEach((value, key) => {
      queryParams.append(key, value)
    })

    const wahaUrl = `${WAHA_API_URL}/api/${params.sessionName}/auth/qr?${queryParams.toString()}`
    console.log(`📡 [WAHA PROXY] Chamando: ${wahaUrl}`)
    ...
    console.log('✅ [WAHA PROXY] QR Code obtido, tamanho:', imageBuffer.byteLength)
