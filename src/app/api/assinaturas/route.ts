...
export async function GET(request: NextRequest) {
  try {
    ...
    let url = `${BACKEND_URL}/api/assinaturas`
    ...
    console.log('Dados recebidos:', data.length, 'assinaturas')
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Criando assinatura:', body)

    const response = await fetch(`${BACKEND_URL}/api/assinaturas`, {
      method: 'POST',
      headers: {
        'Authorization': authHeader,
        'Content-Type': 'application/json',
