...
const ItemSideChat = React.forwardRef<HTMLDivElement, ItemSideChatProps>(({
  ...
  const leadStatusDisplay = React.useMemo<LeadStatusType | null>(() => {
    ...
    // Lógica baseada no atendente:
    // Se NÃO tem atendente → aguardando
    // Se TEM atendente → atendimento
  ...
  useEffect(() => {
    const fetchLeadStatus = async () => {
      try {
        ...
        const response = await fetch(`/api/chats/${encodeURIComponent(chat.id)}/lead`, {
          headers: {
            ...
            <button
              ...
              className={`flex items-center gap-0.5 px-1 py-0.5 rounded-full cursor-pointer transition-all hover:ring-2 ${
                leadStatusDisplay === 'aguardando'
                  ...
                  : 'bg-purple-100 dark:bg-purple-900/20 hover:ring-purple-300'
              }`}
